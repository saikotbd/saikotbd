		...
		<div id="bdt-post-grid-<?php echo esc_attr($id); ?>" class="bdt-post-grid bdt-post-grid-skin-reverse">
	  		<div <?php echo $this->parent->get_render_attribute_string( 'grid-height' ); ?>>
				...
				<?php 
					...
					if($bdt_desktop_class == $settings['columns'] ) {
		  				$bdt_desktop_class = 0;
		  			}
					if($bdt_tablet_class == $settings['columns_tablet'] ) {
		  				$bdt_tablet_class = 0;
		  			}
					if($bdt_mobile_class == $settings['columns_mobile'] ) {
		  				$bdt_mobile_class = 0;
		  			}
		  			...
		  			if ( $bdt_desktop_class % 2 != 0) {
						$bdt_item_class   .= ' bdt-width-1-' . esc_attr($settings['columns']) . '@m bdt-plane-desktop';
		  			} else {
						$bdt_item_class   .= ' bdt-width-1-' . esc_attr($settings['columns']) . '@m bdt-reverse-desktop';
		  			}
					  
					
					if ( $bdt_tablet_class % 2 != 0) {
						$bdt_item_class   .= ' bdt-width-1-' . esc_attr($settings['columns_tablet']) . '@s bdt-plane-tablet';
		  			} else {
						$bdt_item_class   .= ' bdt-width-1-' . esc_attr($settings['columns_tablet']) . '@s bdt-reverse-tablet';
		  			}
					
					if ( $bdt_mobile_class % 2 != 0) {
						$bdt_item_class   .= ' bdt-width-1-' . esc_attr($settings['columns_mobile']) . ' bdt-plane-mobile';
		  			} else {
						$bdt_item_class   .= ' bdt-width-1-' . esc_attr($settings['columns_mobile']) . ' bdt-reverse-mobile';
		  			}
