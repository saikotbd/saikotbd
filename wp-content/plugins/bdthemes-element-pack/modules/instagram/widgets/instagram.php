...
class Instagram extends Module_Base {
    ...
    public function get_style_depends() {
        if ($this->ep_is_edit_mode()) {
            ...
            return ['element-pack-font', 'ep-instagram'];
        }
    }
    ...
    protected function _register_controls() {
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .bdt-instagram .bdt-instagram-item .bdt-overlay .bdt-icon svg' => 'width: {{SIZE}}{{UNIT}};',
                ],
            ]
        );
    ...
    public function get_media($access_token, $account_id) {
        ...
        if ( !$data ) {
            $url    = $this->graph_url . "/$account_id/media?fields=id,media_type,media_url,username,timestamp&access_token=$access_token&limit=100";
            $result = $this->remote_get($url);
            if ( $result->status == 200 ) {
                $data       = $result->body;
